...
const App: React.FC = () => {
    ...
    const fetchPostData = async (postID: string) => {
        try {
            const response = await fetch(`/api/data?id=${postID}`);
            const postData = await response.json();

            if (postData.length > 0) {
                setName(postData[0].name);
                setText(postData[0].text);
                setEditPostId(postData[0]._id);
            }
        } catch (error) {
    ...
    const handleFormSubmit = async (e: FormEvent) => {
        e.preventDefault();
        try {
            const response = await fetch(`/api/data/?id=${editPostId}`, {
                method: "PUT",
                headers: {
